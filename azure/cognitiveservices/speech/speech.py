...
class EmbeddedSpeechConfig(SpeechConfig):
    def __init__(self):
        handle = _spx_handle(0)
...
#        self._properties.set_property_by_name("AZAC-SDK-PROGRAMMING-LANGUAGE", "Python")
    ...
    def disable_telemetry(self):
        self.set_property_by_name("EmbeddedSpeech-TelemetryMode", "FALSE")

    def add_path(self, path):
        c_path=_c_str(path)
        _call_hr_fn(fn=_sdk_lib.embedded_speech_config_add_path, *[self._handle, c_path])

    def set_tts_key(self,key):
        self.set_property(PropertyId.SpeechServiceConnection_SynthModelKey, key)

    def enable_logging(self):
        self.set_property(PropertyId.Speech_LogFilename,"s.log")
        _sdk_lib.diagnostics_log_console_start_logging(1)

    def set_tts_voice(self, voice):
        self.set_property(PropertyId.SpeechServiceConnection_SynthOfflineVoice, voice)

    def tts_disable_audio_sync(self):
        self.set_property(PropertyId.SpeechServiceResponse_SynthesisEventsSyncToAudio, "FALSE")

    def set_tts_path(self, path):
        self.set_property(PropertyId.SpeechServiceConnection_SynthOfflineDataPath, path)
