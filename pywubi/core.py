...
def single_wubi(han, multicode=False, shortest = False):
    '''
    将单个汉字转成五笔编码
    :param han: str ,单个汉字
    ...
    if not multicode:
        return pys[-1] if shortest else pys[0]
    return pys
...
def wubi(hans, multicode=False, single=True, shortest=False):
    '''
    将汉字转换成五笔编码
    ...
    if single:
        ...
        for han in han_list:
            if RE_HANS.match(han):
                res.append(single_wubi(han, multicode, shortest))
            else:
                res.append(han)
    else:
        # 将可以找到编码的汉字进行分词，
